...
The frozen amount can be set above the holder's balance (`setFrozenTokens` is absolute and unchecked), so `frozen` may exceed `total`. A holder can also be frozen outright, which stops it sending anything whatever `free` says.
