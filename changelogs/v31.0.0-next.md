...
From Polymesh 8.1 every fungible Asset can be called from EVM contracts and Ethereum tooling as an ERC-20 token, and from 8.1.1 every NFT collection as an ERC-721 token. Nothing is deployed or registered: the token lives at a precompile address derived from the Asset ID, and calls run the same compliance, restriction and freeze checks as the equivalent extrinsic.
...
collection.evmAddress; // '0x<Asset ID>00090000'; an NFT's ERC-721 tokenId is its `id`
...
- `Assets.getAssetFromEvmAddress({ address })`: accepts any letter case. It throws `ValidationError` for an address without the Asset precompile layout, and for the ERC-20 address of a collection or the ERC-721 address of a fungible Asset, since neither answers on chain. It throws `DataUnavailable` when no Asset has the encoded ID.
...
await asset.controllerTransfer({
  originPortfolio: frozenInvestorPortfolio,
...
Fungible Assets have had allowances since Polymesh 8.0. From 8.1.1 NFTs gain the two approvals ERC-721 defines, so a marketplace or escrow flow no longer needs custody first:
...
From Polymesh 8.1.1 an Asset agent can freeze part of a holder's balance, and the chain refuses to let the holder send frozen tokens. `getAssetBalances()` on an Account or a Portfolio still reported `free` as `total - locked`, so a wallet would show tokens the holder cannot send, and procedures that precheck against `free` (`transferFunds`, `redeem`, `investInOffering`) would build a transaction the chain then rejects.
...
Where the chain itself ignores frozen tokens, the SDK's precheck does too, so no call is refused that the chain would accept:

- **Controller transfers** can seize frozen tokens (that is how an agent recovers them from a frozen holder), so `controllerTransfer` checks against `total - locked`. Its insufficient-balance message now says "unlocked balance" instead of "free balance".
- **Moving funds between an Identity's own Portfolios**, **configuring a dividend distribution** and **launching an offering** reserve against unlocked tokens only on chain, so frozen tokens still count for them.
