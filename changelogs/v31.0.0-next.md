...
| `TxGroup.AdvancedAssetManagement` includes the four holder-freezing extrinsics                                 | Permissions granted with the group before now hold only part of it, so they no longer read back as `AdvancedAssetManagement`. Grant the group again to include freezing            |
