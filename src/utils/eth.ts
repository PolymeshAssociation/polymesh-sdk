 ...
 * @hidden
 *
 * The IDs that `pallet_revive` routes to an Asset precompile. Revive reads the ID from bytes
 *   `[16, 18)` of the address in big-endian order, and reserves bytes `[18, 20)` for its
 *   built-in precompiles. An Asset precompile's address is therefore
 *   `<16 byte AssetId> ++ <2 byte ID> ++ 0x0000`.
 */
...
export function assetIdToPrecompileAddress(assetId: string, kind: AssetPrecompileKind): HexString {
  ...
  address[ASSET_ID_LENGTH] = precompileId >> 8; // big-endian high byte
  address[ASSET_ID_LENGTH + 1] = precompileId & 0xff; // low byte
 ...
 * @hidden
 *
 * Decode the Asset ID and kind of Asset precompile from an Asset precompile's address. The inverse
 * of {@link assetIdToPrecompileAddress}.
 *
 * Asset precompile addresses have the following layout:
 *   `<16 byte AssetId> ++ <2 byte precompile ID> ++ <2 reserved bytes>`
 *
 * The precompile ID is encoded in big-endian order.
 *
...
export function precompileAddressToAssetId(address: string): {
  ...
  // Revive stores the 16-bit precompile ID in bytes 16-17 in big-endian order.
  const precompileId = (bytes[ASSET_ID_LENGTH]! << 8) | bytes[ASSET_ID_LENGTH + 1]!;

  // Bytes 18-19 are reserved for Revive's built-in precompiles and must be zero.
