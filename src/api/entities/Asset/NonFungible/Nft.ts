...
export class Nft extends Entity<NftUniqueIdentifiers, HumanReadable> {
  ...
  /**
   * Approve an Account to transfer this NFT on its holder's behalf, with `transferFunds`. The
   *   non-fungible precompile's equivalent is `approve`
   *
   ...
   *  - `https://example.com/nfts` becomes `https://example.com/nfts/1` if used a base value, but remain unchanged as a local value
   *
   * @note the non-fungible precompile's `tokenURI` (Polymesh 8.1.1) resolves the same two values, but also fills in or appends the ID
  ...
  /**
   * Retrieve the Account approved to transfer this NFT on its holder's behalf, if any. The
   *   non-fungible precompile's equivalent is `getApproved`
   *
