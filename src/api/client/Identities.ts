...
export class Identities {
   ...
   *
   * @deprecated as of chain v8, `createCdd`/`expiry` no longer have any on-chain effect (the
   *   underlying `cdd_register_did`/`cdd_register_did_with_cdd` calls are deprecated and no longer
   *   attach a `CustomerDueDiligence` claim). Use {@link registerDid} instead.
   ...
   *
   * @note the transaction signer must be a DID Registrar
   * @note this creates an {@link AuthorizationRequest | Authorization Requests} which have to be accepted by the `targetAccount` along with the authorization for `RotatingPrimaryKey`.
   *   An {@link api/entities/Account!Account | Account} or {@link Identity} can fetch its pending Authorization Requests by calling {@link api/entities/common/namespaces/Authorizations!Authorizations.getReceived | authorizations.getReceived}.
   *   Also, an Account or Identity can directly fetch the details of an Authorization Request by calling {@link api/entities/common/namespaces/Authorizations!Authorizations.getOne | authorizations.getOne}
   *
   * @note required role:
   ...
   *
   * @note this creates an {@link AuthorizationRequest | Authorization Requests} which have to be accepted by the `targetAccount`
   *   An {@link api/entities/Account!Account | Account} or {@link Identity} can fetch its pending Authorization Requests by calling {@link api/entities/common/namespaces/Authorizations!Authorizations.getReceived | authorizations.getReceived}.
   *   Also, an Account or Identity can directly fetch the details of an Authorization Request by calling {@link api/entities/common/namespaces/Authorizations!Authorizations.getOne | authorizations.getOne}
   */
