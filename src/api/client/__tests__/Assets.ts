...
describe('Assets Class', () => {
  ...
  describe('method: getAssetFromEvmAddress', () => {
    ...
    it('should return the FungibleAsset behind an ERC-20 precompile address', async () => {
      entityMockUtils.configureMocks({
    ...
    it('should return the NftCollection behind an ERC-721 precompile address', async () => {
      entityMockUtils.configureMocks({
        fungibleAssetOptions: { exists: false },
        nftCollectionOptions: { exists: true },
      });
    ...
    it('should throw if the ERC-20 address names an NFT collection', () => {
      entityMockUtils.configureMocks({
        fungibleAssetOptions: { exists: false },
        nftCollectionOptions: { exists: true },
      });
      ...
      return expect(assets.getAssetFromEvmAddress({ address: fungibleAddress })).rejects.toThrow(
        'The supplied address is the ERC-20 precompile address of an NFT collection'
      );
    });

    it('should throw if the ERC-721 address names a fungible Asset', () => {
      entityMockUtils.configureMocks({
        fungibleAssetOptions: { exists: true },
        nftCollectionOptions: { exists: false },
      });
      ...
      return expect(assets.getAssetFromEvmAddress({ address: nftAddress })).rejects.toThrow(
        'The supplied address is the ERC-721 precompile address of a fungible Asset'
      );
    });
