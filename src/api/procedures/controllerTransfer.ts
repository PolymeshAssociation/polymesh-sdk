...
> {
  ...
  const originAssetHolderId = assetHolderLikeToAssetHolderId(origin);
  ...
  const originHolder = assetHolderLikeToAssetHolder(origin, context);

  const [balance] = await originHolder.getAssetBalances({
    assets: [asset],
  });
  ...
  if (available.lt(amount)) {
    throw new PolymeshError({
      ...
      message: 'The origin does not have enough unlocked balance for this transfer',
      data: { available },
    });
  }
