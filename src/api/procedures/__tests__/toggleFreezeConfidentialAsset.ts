...
describe('toggleFreezeConfidentialAsset procedure', () => {
  ...
  beforeEach(() => {
    ...
    when(booleanToBoolSpy).calledWith(true).mockReturnValue(dsMockUtils.createMockBool(true));
    when(booleanToBoolSpy).calledWith(false).mockReturnValue(dsMockUtils.createMockBool(false));
  });
