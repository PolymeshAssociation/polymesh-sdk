...
import { Account, Context, DefaultPortfolio, FungibleAsset } from '~/internal';
...
describe('controllerTransfer procedure', () => {
  ...
  it('should throw if the origin and destination belong to the same Identity', () => {
    ...
    const proc = procedureMockUtils.getInstance<Params, void, Storage>(mockContext, buildStorage());
  ...
  describe('with an Account origin', () => {
    ...
    beforeEach(() => {
      ...
      assetHolderIdToMeshAssetHolderSpy.mockImplementation(holderId =>
        holderId === investorAddress
          ? rawInvestorHolder
          : (rawPortfolioId as unknown as PolymeshPrimitivesAssetAssetHolder)
      );
      ...
      entityMockUtils.configureMocks({
        accountOptions: {
          getIdentity: entityMockUtils.getIdentityInstance({ did: 'investorDid' }),
        },
      });
      investorAccount = entityMockUtils.getAccountInstance({
      ...
      assetHolderLikeToAssetHolderSpy.mockReturnValue(investorAccount);
    });

    it("should seize from the Account's unlocked balance", async () => {
      const proc = procedureMockUtils.getInstance<Params, void, Storage>(
        mockContext,
        buildStorage()
      );
      ...
      expect(investorAccount.getAssetBalances).toHaveBeenCalledWith({ assets: [asset] });
      expect(result).toEqual({
        transaction: mockContext.polymeshApi.tx.asset.controllerTransfer,
        args: [rawAssetId, rawAmount, rawInvestorHolder, rawDestinationKind],
        resolver: undefined,
      });
    });

    it('should throw an error if the Account does not have enough unlocked balance', () => {
      const proc = procedureMockUtils.getInstance<Params, void, Storage>(
        mockContext,
        buildStorage()
      );
      ...
      ).rejects.toThrow('The origin does not have enough unlocked balance for this transfer');
    });

    it('should throw if the Account and the destination belong to the same Identity', () => {
      // `getAssetHolderDid` reads the Identity of a fresh Account instance
      entityMockUtils.configureMocks({
        accountOptions: { getIdentity: entityMockUtils.getIdentityInstance({ did: 'someDid' }) },
      });
      ...
      return expect(
        prepareControllerTransfer.call(proc, { asset, origin: ownAccount, amount })
      ).rejects.toThrow('The origin and destination must belong to different Identities');
    });
  });
