...
describe('consumeRotatePrimaryKeyWithSecondary procedure', () => {
  ...
  test('should throw an error if the Authorization Request is expired', () => {
    ...
    return expect(
      prepareConsumeRotatePrimaryKeyToSecondaryAuthorization.call(proc, {
  ...
  test('should add a rotatePrimaryKeyToSecondary transaction to the queue if the target is an Account', async () => {
    ...
    await prepareConsumeRotatePrimaryKeyToSecondaryAuthorization.call(proc, {
      authRequest: new AuthorizationRequest(
        {
          target,
          issuer,
