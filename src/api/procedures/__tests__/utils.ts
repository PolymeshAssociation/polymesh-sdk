...
describe('getControllerTransferOrigin', () => {
  it('should return `origin`', () => {
    expect(getControllerTransferOrigin({ origin: 'someAddress' })).toBe('someAddress');
  });

  it('should return the deprecated `originPortfolio`', () => {
    expect(getControllerTransferOrigin({ originPortfolio: 'someDid' })).toBe('someDid');
  });

  it('should throw if both are passed', () => {
    expect(() =>
      getControllerTransferOrigin({ origin: 'someAddress', originPortfolio: 'someDid' })
    ).toThrow(
      expect.objectContaining({
        code: ErrorCode.ValidationError,
        message: 'Pass "origin" or the deprecated "originPortfolio", not both',
      })
    );
  });

  it('should throw if neither is passed', () => {
    expect(() => getControllerTransferOrigin({})).toThrow(
      expect.objectContaining({
        code: ErrorCode.ValidationError,
        message: 'Pass "origin", the Portfolio or Account to transfer from',
      })
    );
  });
});
...
describe('assertControllerTransferDestinationAccepted', () => {
  ...
  beforeEach(() => {
    jest
      ...
      .mockReturnValue(dsMockUtils.createMockAssetId('0x12341234123412341234123412341234'));
  });
...
describe('authorization request validations', () => {
  ...
  describe('assertJoinOrRotateAuthorizationValid', () => {
    ...
    it('should throw when the target is an Identity', () => {
      const mockIssuer = entityMockUtils.getIdentityInstance({ exists: true });
