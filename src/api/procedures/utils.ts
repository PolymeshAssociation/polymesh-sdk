 ...
 * @hidden
 *
 * Read where a controller transfer takes from, given as `origin` or as the deprecated
 *   `originPortfolio`. The two mean the same thing, so passing both says nothing about which was
 *   meant
 */
export function getControllerTransferOrigin(args: {
  origin?: AssetHolderLike;
  originPortfolio?: PortfolioLike;
}): AssetHolderLike {
  const { origin, originPortfolio } = args;

  if (origin !== undefined && originPortfolio !== undefined) {
    throw new PolymeshError({
      code: ErrorCode.ValidationError,
  ...
  const holder = origin ?? originPortfolio;
